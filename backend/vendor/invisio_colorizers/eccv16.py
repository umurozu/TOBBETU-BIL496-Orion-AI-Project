...
class ECCVGenerator(BaseColor):
    def __init__(self, norm_layer=nn.BatchNorm2d, cluster_points_path=None):
        super(ECCVGenerator, self).__init__()

        cluster_path = Path(cluster_points_path or "cluster_points.npy")
        self.register_buffer(
            "data",
            torch.from_numpy(np.load(cluster_path)).float(),
            persistent=False,
        )
...
def eccv16(pretrained=True, number_of_inputs=1, weights_path=None, cluster_points_path=None):
    model = ECCVGenerator(cluster_points_path=cluster_points_path)
    if(pretrained):
        if weights_path:
