...
import { shareDirectToCommunity } from '../../services/api';
...
export const DirectShareModal: React.FC<DirectShareModalProps> = ({ imageToShare, aiOperation, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!accessToken || !imageToShare) return;

        try {
            setIsSubmitting(true);
            setError('');
            
            // Convert data URL or object URL to Blob
            const response = await fetch(imageToShare);
            const blob = await response.blob();

            await shareDirectToCommunity(blob, caption, aiOperation, accessToken);
